"""Settings for the literature Q&A (RAG) package.

This module hosts package-specific Pydantic settings used by the
literature Q&A agent (Docs / RAG / agents). It also acts as the **primary
aggregate settings module** for ``evoscholar``: the top-level ``Settings``
class lives here and aggregates per-package settings, including
``ResearchSettings`` from ``evoscholar.iterative_search.settings``.

Per-package ownership:

* ``AnswerSettings`` / ``ParsingSettings`` / ``ChunkingOptions`` /
  ``MultimodalOptions`` / ``PromptSettings`` / ``IndexSettings`` /
  ``AgentSettings`` → **this package** (``literature_qa.settings``)
* ``ResearchSettings`` / ``AsyncContextSerializer`` / ``_FormatDict`` →
  ``iterative_search.settings`` (re-exported via this module for the
  aggregated ``Settings`` class)

Backward compatibility:
* ``from evoscholar.literature_qa.settings import Settings`` works
* ``from evoscholar.literature_qa.settings import AnswerSettings`` etc. works
"""
...
from evoscholar._ldp_shims import (
    HAS_LDP_INSTALLED,
    Agent,
    HTTPAgentClient,
    MemoryAgent,
...
from evoscholar.iterative_search.settings import (
...
# Re-export iterative_search-owned names so callers can do
# ``from evoscholar.literature_qa.settings import ResearchSettings``
__all__ = [
    "AnswerSettings",
    "AgentSettings",
    "AsyncContextSerializer",
    ...
    "default_pdf_parser_configurator",
    "get_settings",
...
# ---------------------------------------------------------------------------
# ParsingSettings (with ChunkingOptions / MultimodalOptions)
# ---------------------------------------------------------------------------
...
class IndexSettings(BaseModel):
    ...
    recurse_subdirectories: bool = Field(
        default=True, description="Whether to recurse into subdirectories when indexing sources.",
    )
...
# ---------------------------------------------------------------------------
# LLM model list settings helper
# ---------------------------------------------------------------------------
...
class Settings(BaseSettings):
    ...
    def from_name(
        ...
        pkg_config_path = (
            ...
            importlib.resources.files(evoscholar.configs)
            / f"{config_name}.json"
        )
        if user_config_path.exists():
            ...
            try:
                if pkg_config_path.is_file():
                    json_path = cast_path(pkg_config_path)
    ...
    async def make_ldp_agent(
        ...
        agent_cls = cast_type("type[Agent]", locate(agent_type))
...
def cast_path(v: object) -> pathlib.Path:
    """Local thin cast helper to keep ``Settings.from_name`` annotation-clean."""
    from typing import cast as _cast

    return _cast(pathlib.Path, v)


def cast_type(annotation: str, value: object) -> object:
    """Local thin cast helper to keep ``Settings`` method annotations clean."""
    from typing import cast as _cast

    return _cast(annotation, value)
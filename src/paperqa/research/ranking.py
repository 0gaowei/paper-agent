...
from .models import AcademicPaper, RelevanceTier
...
async def _embed_texts(embedding_model: Any, texts: list[str]) -> list[list[float]]:
...
def _embed_texts_sync(embedding_model: Any, texts: list[str]) -> list[list[float]]:
    if hasattr(embedding_model, "embed_documents"):
        result = embedding_model.embed_documents(texts)
    elif hasattr(embedding_model, "embed"):
        result = embedding_model.embed(texts)
...
def score_papers(
    ...
    try:
        ...
        vectors = _embed_texts_sync(embedding_model, [query, *document_texts])
...
async def ascore_papers(
    ...
    try:
        ...
        vectors = await _embed_texts(embedding_model, [query, *document_texts])
        relevance_scores = [_cosine(vectors[0], vector) for vector in vectors[1:]]
    except Exception:
        relevance_scores = [_lexical_relevance(query, paper) for paper in papers]
    _assign_scores(papers, relevance_scores, config)
